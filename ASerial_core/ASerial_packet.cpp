...
size_t ASerialPacket::GetNeedPacketBufSize(uint8_t *data_array, int data_num)
{
  ...
  if (GetMode() == MODE_DEVICE) {
    ...
    size += 4;  // スタートフラグ(1) + ターゲットデバイスID(1) + データ数(1) +
                // コマンド(1)
  }
...
int ASerialPacket::ReadPacketData(uint8_t _indata,
                                  ASerialDataStruct::ASerialData *data_buf_pt)
{
  ...
  static uint8_t check_data_buf[2] = {
      0};  // 2データ分のデータを読むためのバッファ

  if (_indata == DO_FLAG) {       // スタートフラグ
    if (m_read_packet == true) {  // パケットの読み飛ばし発生警告
      m_lase_error_code =
          static_cast<uint16_t>(ASerial::ErrorCodeList::WARNING_READ_SKIP);
    }
    ...
    if (m_add_flag == true) {
      ...
      m_read_packet = false;
      m_lase_error_code =
          static_cast<uint16_t>(ASerial::ErrorCodeList::ERR_ADDFLAG_CASCADE);
    }
    else {
      ret_st = 0;
      m_add_flag = true;
      ...
      switch (step) {
        ...
        case 1:  // 送信データ数
          ...
          if (m_from_controller_data_buf.data_num > DATA_NUM_MAX) {
            m_lase_error_code = static_cast<uint16_t>(
                ASerial::ErrorCodeList::ERR_DATA_COUNT_INFO_OVER);
            m_error_flag = true;
            ret_st = -1;
          }
          else {
        ...
        case 2:  // コマンド
          ...
          if ((m_from_controller_data_buf.command != RESERVED_COMMAND_RESET) &&
              (m_from_controller_data_buf.command !=
               RESERVED_COMMAND_GET_INFO) &&
              (m_from_controller_data_buf.target_device_id != m_device_id)) {
            m_error_flag = true;
            m_lase_error_code = static_cast<uint16_t>(
                ASerial::ErrorCodeList::ERR_DEVICE_ID_MISMATCH);
            ret_st = -1;
          }
          else {
            ret_st = 0;
          ...
          if (m_from_controller_data_buf.data_num <=
              0) {  // データが0個ならチェックデータ読み取りへ
            step = 4;
          }
          else {
            step = 3;
        ...
        case 5:  // チェックデータ2
          ...
          uint16_t check_data =
              (((uint16_t)check_data_buf[0] << 8) | check_data_buf[1]);
          if (check_data_sum != check_data) {
            ret_st = -1;
            m_lase_error_code = static_cast<uint16_t>(
                ASerial::ErrorCodeList::ERR_CHECK_DATA_MISMATCH);
          }
          else {
            ret_st = 1;
            m_read_packet = false;
...
int ASerialPacket::MakePacketData(uint8_t *to_device_data, int data_num,
                                  uint8_t command, uint8_t *data_packet_out)
{
  if (GetMode() !=
      MODE_CONTROLLER) {  // デバイスモード時にコントローラのパケットを作成を制限
    return -1;
  }
  ...
  ++index;
  CheckDataNeedAddFlag(
      m_target_device_id, data_packet_out, &index);  // ターゲットデバイスID
  ++index;
...
int ASerialPacket::MakePacketData(uint8_t *to_controller_data, int data_num,
                                  uint8_t *data_packet_out)
{
  if (GetMode() !=
      MODE_DEVICE) {  // コントローラモード時にデバイスのパケットを作成を制限
    return -1;
  }
...
int ASerialPacket::CheckDataNeedAddFlag(uint8_t data, uint8_t *data_array,
                                        int *_index)
{
